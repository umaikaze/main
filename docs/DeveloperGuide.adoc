...
[NOTE]
As a more OOP model, we can store a `Tag` list in `Pet Tracker`, which `Pet` can reference. This would allow `Pet Tracker` to only require one `Tag` object per unique `Tag`, instead of each `Pet` needing their own `Tag` object. An example of how such a model may look like is given below. +
 +
