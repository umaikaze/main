...
Given below is an example usage scenario that demonstrates how the `Schedule` system is integrated with the `Pet` system

Step 1. The user launches the application with a pet stored: `Garfield`. `ObservableList<Slot>`
in `Schedule` is occupied with a single instance of `Slot`, whose parameters are: `petName: Garfield`,
`dateTime: 11/4/2020 1200`, `duration: 40`. Upon launch, a `PetListChangeListener` will be attached to `internalList` of
`UniquePetList`.

Step 2. The user executes `deletepet 1` command to delete `Garfield` from the `UniquePetList`. When the pet is deleted,
function `onChanged` in `PetListChangeListener` is called. This function calls `removeExcessSlot` within the
`PetListChangeListener` for each pet that is being removed.
