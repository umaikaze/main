...
import static seedu.address.testutil.slot.TypicalSlots.garfieldSlot;
...
public class JsonAdaptedSlotTest {
    ...
    private static final String VALID_NAME = garfieldSlot.getPet().getName().fullName;
    private static final String VALID_DATE_TIME = garfieldSlot.getDateTime().format(DATETIME_FORMAT);
    private static final String VALID_DURATION = garfieldSlot.getDuration().toString();

    @Test
    public void toModelType_validSlotDetails_returnsSlot() throws Exception {
        JsonAdaptedSlot slot = new JsonAdaptedSlot(garfieldSlot);
        assertEquals(garfieldSlot, slot.toModelType(typicalPetTracker));
    }
